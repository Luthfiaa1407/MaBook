    ...
    <div class="w-11/12 max-w-[1200px] mx-auto">
        <div class="flex items-center justify-between gap-2 p-3">
            ...
            <div class="flex text-mabook-light text-lg justify-center items-center font-crimson gap-3">
                ...
                <?php if ($isLoggedIn): ?>
                    <a href="<?= url('user/favoritku.php') ?>" class="hover:text-mabook-midtone p-2 group relative text-center">
                        Favoritku
                        <div class="absolute bg-mabook-midtone w-0 left-0 bottom-0 h-[2px] group-hover:w-full duration-200"></div>
                    </a>
                <?php endif; ?>
            ...
            <div>
                <form action="<?= url('collection.php') ?>" method="GET">
                    <div class="flex items-center">
                        <input value="<?= htmlspecialchars($search) ?>" name="search" type="text" class="bg-[#1a120b] p-2 text-mabook-light font-crimson" placeholder="Cari buku, penulis...">
            ...
            <div class="flex gap-2 items-center justify-center">
                <?php if ($isLoggedIn) : ?>
                    ...
                    <?php else: ?>
                        <div class="text-mabook-light text-sm italic">Hello, <?= htmlspecialchars($loggedUser['name'] ?? 'User') ?></div>
                    <?php endif; ?>
